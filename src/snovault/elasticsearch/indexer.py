...
#   def index(request):
#       if not request.registry['indexer_uuids']:
#           print('indexer', 'no uuids')
#           indexer_not_running(request)
#       else:
#           print('indexer', 'has uuids', len(request.registry['indexer_uuids']))
...
def index_worker(request):
    ...
    print('index_worker listener', request.registry.settings.get('indexer_uuids', 'no uuids key']))
...
@view_config(route_name='index', request_method='POST', permission="index")
def index(request):
    print('index listener', request.registry.settings.get('indexer_uuids', 'no uuids key']))
    '''Indexer Listener'''
    ...
    if len(invalidated) > SEARCH_MAX:
        ...
        # indexer_stage_two has a return case.  If invalidated comes back as
