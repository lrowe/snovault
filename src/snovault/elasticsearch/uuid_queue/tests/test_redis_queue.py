...
class TestRedisQueueMeta(TestCase):
    ...
    def test_add_errors(self):
        ...
        # pylint: disable=protected-access
        self.assertEqual(
            int(self.queue_meta._client.get(self.queue_meta._key_errorscount)),
            len(errors_to_add)
        )
    ...
    def test_set_args(self):
        ...
        self.queue_meta._client.set(self.queue_meta._key_isrunning, 'false')
        self.queue_meta._client.set(self.queue_meta._key_successescount, 10)
        worker_id = 'test-worker-id-a'
        self.queue_meta.add_worker_conn(worker_id)
        self.queue_meta.set_args()
        self.assertEqual(
            int(self.queue_meta._client.get(self.queue_meta._key_addedcount)),
            0
        )
        self.assertEqual(
            int(self.queue_meta._client.get(self.queue_meta._key_errorscount)),
        ...
        self.assertEqual(
            self.queue_meta._client.get(self.queue_meta._key_isrunning),
            'true'
        )
        self.assertEqual(
            int(self.queue_meta._client.get(self.queue_meta._key_successescount)),
            0
        )
        self.assertEqual(
            len(self.queue_meta._client.lrange(self.queue_meta._key_workers, 0, -1)),
            0
        )
...
class TestRedisQueue(TestCase):
    ...
    def test_init_basic(self):
        '''Test basic queue meta init'''
        self.assertEqual(self.queue.queue_name, self.queue_options['queue_name'])
        # pylint: disable=protected-access
